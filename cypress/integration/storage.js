    ...
    describe("Storage tests, storage: " + flag, () => {
        ...
        describe("basic setting", () => {
            it("Checks if setValueInStorage function stores a value correctly", () => {
                hp.haltAndClearStorage(() => {
                    initMain(flag);
                    Atpl._internals.setValueInStorage(key, valueToStore);
            ...
            it("Checks if getValueFromStorage function gets a value correctly", () => {
                hp.haltAndClearStorage(() => {
                    ...
                    Atpl._internals.setValueInStorage(key, valueToStore);
                    expect(isNone ? undefined : valueToStore).to.equal(Atpl._internals.getValueFromStorage(key));
                });
            });
            it("Checks if getValueFromStorage function can not get a value if it does not exist", () => {
                hp.haltAndClearStorage(() => {
                    initMain(flag);
                    expect(isNone ? undefined : null).to.equal(Atpl._internals.getValueFromStorage(key));
                });
            });
            it("Checks if removeValueFromStorage function removes a value correctly", () => {
                hp.haltAndClearStorage(() => {
                    ...
                    Atpl._internals.setValueInStorage(key, valueToStore);
                    expect(isNone ? undefined : valueToStore).to.equal(Atpl._internals.getValueFromStorage(key));
                    Atpl._internals.removeValueFromStorage(key);
                    expect(isNone ? undefined : null).to.equal(Atpl._internals.getValueFromStorage(key));
                });
            });
        });
        ...
        describe("uselocalstorage: false ", () => {
            it("Checks if setValueInStorage function stores a cookie correctly", () => {
                hp.haltAndClearStorage(() => {
                    initMain(flag);
                    Atpl._internals.setValueInStorage(key, valueToStore, false);
                    cy.getLocalStorage(`${hp.appKey}/${key}`).then((value) => {
                        expect(value).to.equal(null);
                    });
                    if (isNone || isLocal) {
            ...
            it("Checks if getValueFromStorage function gets a value correctly", () => {
                hp.haltAndClearStorage(() => {
                    ...
                    Atpl._internals.setValueInStorage(key, valueToStore, false);
                    if (isCookie) {
                        expect(isNone || isLocal ? undefined : null).to.equal(Atpl._internals.getValueFromStorage(key, !!isCookie));
                    }
                    expect(isNone || isLocal ? undefined : valueToStore).to.equal(Atpl._internals.getValueFromStorage(key, false));
                });
            });
            it("Checks if getValueFromStorage function can not get a value if it does not exist", () => {
                hp.haltAndClearStorage(() => {
            ...
            it("Checks if removeValueFromStorage function removes a value correctly", () => {
                hp.haltAndClearStorage(() => {
                    ...
                    Atpl._internals.setValueInStorage(key, valueToStore, false);
                    expect(isNone || isLocal ? undefined : valueToStore).to.equal(Atpl._internals.getValueFromStorage(key, false));
                    Atpl._internals.removeValueFromStorage(key, false);
                    expect(isNone || isLocal ? undefined : null).to.equal(Atpl._internals.getValueFromStorage(key, false));
                });
            });
        });
        ...
        describe("useRawKey: true", () => {
            it("Checks if setValueInStorage function stores a value correctly", () => {
                hp.haltAndClearStorage(() => {
                    initMain(flag);
                    Atpl._internals.setValueInStorage(key, valueToStore, undefined, true);
            ...
            it("Checks if getValueFromStorage function gets a value correctly", () => {
                hp.haltAndClearStorage(() => {
                    initMain(flag);
                    Atpl._internals.setValueInStorage(key, valueToStore, undefined, true);
                    expect(isNone ? undefined : valueToStore).to.equal(Atpl._internals.getValueFromStorage(key, undefined, true));
                });
            });
            it("Checks if getValueFromStorage function can not get a value if it does not exist", () => {
                hp.haltAndClearStorage(() => {
                    initMain(flag);
                    expect(isNone ? undefined : null).to.equal(Atpl._internals.getValueFromStorage(key, undefined, true));
                });
            });
            it("Checks if removeValueFromStorage function removes a value correctly", () => {
                hp.haltAndClearStorage(() => {
                    ...
                    Atpl._internals.setValueInStorage(key, valueToStore, undefined, true);
                    expect(isNone ? undefined : valueToStore).to.equal(Atpl._internals.getValueFromStorage(key, undefined, true));
                    Atpl._internals.removeValueFromStorage(key, undefined, true);
                    expect(isNone ? undefined : null).to.equal(Atpl._internals.getValueFromStorage(key, undefined, true));
                });
            });
        });
        ...
        describe("uselocalstorage: false, useRawKey: true", () => {
            it("Checks if setValueInStorage function stores a value correctly", () => {
                hp.haltAndClearStorage(() => {
                    initMain(flag);
                    Atpl._internals.setValueInStorage(key, valueToStore, false, true);
                    cy.getLocalStorage(`${key}`).then((value) => {
                        expect(value).to.equal(null);
                    });
                    cy.getLocalStorage(`${hp.appKey}/${key}`).then((value) => {
            ...
            it("Checks if getValueFromStorage function gets a value correctly", () => {
                hp.haltAndClearStorage(() => {
                    initMain(flag);
                    Atpl._internals.setValueInStorage(key, valueToStore, false, true);
                    expect(isNone || isLocal ? undefined : valueToStore).to.equal(Atpl._internals.getValueFromStorage(key, false, true));
                    expect(isNone || isLocal ? undefined : null).to.equal(Atpl._internals.getValueFromStorage(key, false));
                });
            });
            it("Checks if getValueFromStorage function can not get a value if it does not exist", () => {
                hp.haltAndClearStorage(() => {
                    initMain(flag);
                    expect(isNone || isLocal ? undefined : null).to.equal(Atpl._internals.getValueFromStorage(key, false, true));
                });
            });
            it("Checks if removeValueFromStorage function removes a value correctly", () => {
                hp.haltAndClearStorage(() => {
                    ...
                    Atpl._internals.setValueInStorage(key, valueToStore, false, true);
                    expect(isNone || isLocal ? undefined : valueToStore).to.equal(Atpl._internals.getValueFromStorage(key, false, true));
                    Atpl._internals.removeValueFromStorage(key, false, true);
                    expect(isNone || isLocal ? undefined : null).to.equal(Atpl._internals.getValueFromStorage(key, false, true));
                });
            });
        });
