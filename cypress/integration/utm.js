...
describe("UTM tests ", () => {
    ...
    it("Checks if utm tag works in multi instancing", () => {
        hp.haltAndClearStorage(() => {
            ...
            initMulti("Atpl_2", "?utm_ss=hehe2", { ss: true });

            // utm object provided with inappropriate query
            initMulti("Atpl_4", "?utm_source=hehe4", { ss: true });

            // utm object not provided with default query
            initMulti("Atpl_3", "?utm_source=hehe3", undefined);

            // utm object not provided with inappropriate query
            initMulti("Atpl_5", "?utm_ss=hehe5", undefined);
    ...
    it("Checks if multi instancing works plus", () => {
        hp.haltAndClearStorage(() => {
            ...
            initMulti("Atpl_multi_1", "?utm_source=hehe&utm_medium=hehe1&utm_campaign=hehe2&utm_term=hehe3&utm_content=hehe4&fdsjhflkjhsdlkfjhsdlkjfhksdjhfkj+dsf;jsdlkjflk+=skdjflksjd=fksdfl;sd=sdkfmk&&&", undefined);

            // utm object given that includes 2 default 1 custom, full plus custom query + gabledeboop
            initMulti("Atpl_multi_2", "?utm_source=hehe&utm_medium=hehe1&utm_campaign=hehe2&utm_term=hehe3&utm_content=hehe4&utm_sthelse=hehe5&fdsjhflkjhsdlkfjhsdlkjfhksdjhfkj+dsf;jsdlkjflk+=skdjflksjd=fksdfl;sd=sdkfmk&&&", { source: true, term: true, sthelse: true });

            // empty init, garbage query + 1 default
            initMulti("Atpl_multi_3", "?dasdashdjkhaslkjdhsakj=dasmndlask=asdkljska&&utm_source=hehe", undefined);

            // full default utm obj + custom 1, full query + 1
            initMulti("Atpl_multi_4", "?utm_source=hehe&utm_medium=hehe1&utm_campaign=hehe2&utm_term=hehe3&utm_content=hehe4&utm_next=hehe5", { source: true, medium: true, campaign: true, term: true, content: true, next: true });

            // full default utm obj + custom 1, no query
            initMulti("Atpl_multi_5", "", { source: true, medium: true, campaign: true, term: true, content: true, next: true });
            ...
            cy.fetch_local_request_queue("Atpl_multi_1").then((rq) => {
                const custom = JSON.parse(rq[0].user_details).custom;
                hp.validateDefaultUtmTags(custom, "hehe", "hehe1", "hehe2", "hehe3", "hehe4");
            });

            // check if custom utm tags works for multi 2
            cy.fetch_local_request_queue("Atpl_multi_2").then((rq) => {
                const custom = JSON.parse(rq[0].user_details).custom;
            ...
            cy.fetch_local_request_queue("Atpl_multi_3").then((rq) => {
                const custom = JSON.parse(rq[0].user_details).custom;
                hp.validateDefaultUtmTags(custom, "hehe", "", "", "", "");
            });

            // check if custom utm tags works for multi 4
            cy.fetch_local_request_queue("Atpl_multi_4").then((rq) => {
                const custom = JSON.parse(rq[0].user_details).custom;
                hp.validateDefaultUtmTags(custom, "hehe", "hehe1", "hehe2", "hehe3", "hehe4");
            ...
            cy.fetch_local_request_queue("Atpl_multi_5").then((rq) => {
                expect(rq.length).to.eq(0);
            });
        });
    });
    it("Checks if multi instancing works plus plus", () => {
        hp.haltAndClearStorage(() => {
            ...
            initMulti("Atpl_multi_next_1", "?utm_sourcer=hehe&utm_mediumr=hehe1&utm_campaignr=hehe2&utm_rterm=hehe3&utm_corntent=hehe4&fdsjhflkjhsdlkfjhsdlkjfhksdjhfkj+dsf;jsdlkjflk+=skdjflksjd=fksdfl;sd=sdkfmk&&&", undefined);

            // utm object default, custom query + gabledeboop
            initMulti("Atpl_multi_next_2", "?utm_sourcer=hehe&utm_mediumr=hehe1&utm_campaignr=hehe2&utm_rterm=hehe3&utm_corntent=hehe4&fdsjhflkjhsdlkfjhsdlkjfhksdjhfkj+dsf;jsdlkjflk+=skdjflksjd=fksdfl;sd=sdkfmk&&&", { source: true, medium: true, campaign: true, term: true, content: true });

            // custom utm object, custom query + gabledeboop
            initMulti("Atpl_multi_next_3", "?utm_sauce=hehe&utm_pan=hehe2&dasdashdjkhaslkjdhsakj=dasmndlask=asdkljska&&utm_source=hehe", { sauce: true, pan: true });
            ...
            // check if custom utm tags works for multi 1
            cy.fetch_local_request_queue("Atpl_multi_next_1").then((rq) => {
                expect(rq.length).to.eq(0);
            });

            // check if custom utm tags works for multi 2
            cy.fetch_local_request_queue("Atpl_multi_next_2").then((rq) => {
                expect(rq.length).to.eq(0);
            });

            // check if custom utm tags works for multi 3
            cy.fetch_local_request_queue("Atpl_multi_next_3").then((rq) => {
                const custom = JSON.parse(rq[0].user_details).custom;
                hp.validateDefaultUtmTags(custom, undefined, undefined, undefined, undefined, undefined);
