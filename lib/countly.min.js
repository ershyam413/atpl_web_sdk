(function(la,L){"object"===typeof exports&&"undefined"!==typeof module?L(exports):"function"===typeof define&&define.amd?define(["exports"],L):(la="undefined"!==typeof globalThis?globalThis:la||self,L(la.Atpl=la.Atpl||{}))})(this,function(la){function L(l){"@babel/helpers - typeof";return L="function"==typeof Symbol&&"symbol"==typeof Symbol.iterator?function(k){return typeof k}:function(k){return k&&"function"==typeof Symbol&&k.constructor===Symbol&&k!==Symbol.prototype?"symbol":typeof k},L(l)}
...
k.async=!0;q.async=!0;k.src=p.customSourceBoomerang||wb.BOOMERANG_SRC;q.src=p.customSourceAtplBoomerang||wb.CLY_BOOMERANG_SRC;document.getElementsByTagName("head")[0].appendChild(k);document.getElementsByTagName("head")[0].appendChild(q);var v=!1,G=!1;k.onload=function(){v=!0};q.onload=function(){G=!0};var Q=0,O=setInterval(function(){Q+=50;if(v&&G||1500<=Q){if(p.debug){var U="BoomerangJS loaded:["+v+"], Atpl_boomerang loaded:["+G+"].";v&&G?console.log("[DEBUG] "+U):console.warn("[WARNING] "+
...
k.appendChild(q);l=document.createElement("div");l.setAttribute("id","cly-loader");document.body.onload=function(){if(p.showLoaderProtection)Aa()&&console.warn("[WARNING] [Atpl] showloader, Loader is already on");else try{document.body.appendChild(l)}catch(v){Aa()&&console.error("[ERROR] [Atpl] showLoader, Body is not loaded for loader to append: "+v)}}}l.style.display="block"}}function Aa(){return p&&p.debug&&"undefined"!==typeof console?!0:!1}function Fb(){if(x){p.showLoaderProtection=!0;
var l=document.getElementById("cly-loader");l&&(l.style.display="none")}}var x="undefined"!==typeof window,p=globalThis.Atpl||{};p.features="sessions events views scrolls clicks forms crashes attribution users star-rating location apm feedback remote-config".split(" ");var M={NPS:"[CLY]_nps",SURVEY:"[CLY]_survey",STAR_RATING:"[CLY]_star_rating",VIEW:"[CLY]_view",ORIENTATION:"[CLY]_orientation",ACTION:"[CLY]_action"},Tb=Object.values(M),d={ERROR:"[ERROR] ",WARNING:"[WARNING] ",INFO:"[INFO] ",DEBUG:"[DEBUG] ",
VERBOSE:"[VERBOSE] "},wb={BOOMERANG_SRC:"https://cdn.jsdelivr.net/npm/Atpl-sdk-web@latest/plugin/boomerang/boomerang.min.js",CLY_BOOMERANG_SRC:"https://cdn.jsdelivr.net/npm/Atpl-sdk-web@latest/plugin/boomerang/Atpl_boomerang.js"},S=Object.freeze({errorCount:"cly_hc_error_count",warningCount:"cly_hc_warning_count",statusCode:"cly_hc_status_code",errorMessage:"cly_hc_error_message"});p.q=p.q||[];p.onload=p.onload||[];var Gb=/^(((([^:\/#\?]+:)?(?:(\/\/)((?:(([^:@\/#\?]+)(?::([^:@\/#\?]+))?)@)?(([^:\/#\?\]\[]+|\[[^\/\]@#?]+\])(?::([0-9]+))?))?)?)?((\/?(?:[^\/\?#]+\/+)*)([^\?#]*)))?(\?[^#]+)?)(#.*)?/,
...
n(r,R))}},!0)):(b(d.ERROR,"fetch_remote_config_explicit, Remote config requires explicit consent"),n&&n(Error("Remote config requires explicit consent"),R))}function Q(){f.ignore_prefetch&&x&&"undefined"!==typeof document.visibilityState&&"prerender"===document.visibilityState&&(f.ignore_visitor=!0);f.ignore_bots&&Ab()&&(f.ignore_visitor=!0)}function O(){0<J.length&&(b(d.DEBUG,"Flushing events"),N({events:JSON.stringify(J)}),J=[],w("cly_event",J))}function U(a,c){if(x)if(document.getElementById("Atpl-feedback-sticker-"+
a._id))b(d.ERROR,"Widget with same ID exists");else try{var e=document.createElement("div");e.className="Atpl-iframe-wrapper";e.id="Atpl-iframe-wrapper-"+a._id;var h=document.createElement("span");h.className="Atpl-feedback-close-icon";h.id="Atpl-feedback-close-icon-"+a._id;h.innerText="x";var m=document.createElement("iframe");m.name="Atpl-feedback-iframe";m.id="Atpl-feedback-iframe";m.src=f.url+"/feedback?widget_id="+a._id+"&app_key="+f.app_key+"&device_id="+f.device_id+"&sdk_version=23.12.0";
document.body.appendChild(e);e.appendChild(h);e.appendChild(m);A(document.getElementById("Atpl-feedback-close-icon-"+a._id),"click",function(){document.getElementById("Atpl-iframe-wrapper-"+a._id).style.display="none";document.getElementById("cfbg").style.display="none"});if(c){var g=document.createElementNS("http://www.w3.org/2000/svg","svg");g.id="feedback-sticker-svg";g.setAttribute("aria-hidden","true");g.setAttribute("data-prefix","far");g.setAttribute("data-icon","grin");g.setAttribute("class",
...
var r=document.createElement("span");r.innerText=a.trigger_button_text;var t=document.createElement("div");t.style.color=7>a.trigger_font_color.length?"#"+a.trigger_font_color:a.trigger_font_color;t.style.backgroundColor=7>a.trigger_bg_color.length?"#"+a.trigger_bg_color:a.trigger_bg_color;t.className="Atpl-feedback-sticker  "+a.trigger_position+"-"+a.trigger_size;t.id="Atpl-feedback-sticker-"+a._id;g.appendChild(n);t.appendChild(g);t.appendChild(r);document.body.appendChild(t);var I=document.getElementById("smileyPathInStickerSvg");
I&&(I.style.fill=7>a.trigger_font_color.length?"#"+a.trigger_font_color:a.trigger_font_color);A(document.getElementById("Atpl-feedback-sticker-"+a._id),"click",function(){document.getElementById("Atpl-iframe-wrapper-"+a._id).style.display="block";document.getElementById("cfbg").style.display="block"})}else document.getElementById("Atpl-iframe-wrapper-"+a._id).style.display="block",document.getElementById("cfbg").style.display="block"}catch(C){b(d.ERROR,"Somethings went wrong while element injecting process: "+
...
0):void 0!==p.device_id&&(D=0));w("cly_id",this.device_id);w("cly_id_type",D);if(c){ka={};for(var n in this.utm)e[n]?(this.userData.set("utm_"+n,e[n]),ka[n]=e[n]):this.userData.unset("utm_"+n);this.userData.save()}ya();setTimeout(function(){Ua();f.remote_config&&f.fetch_remote_config(f.remote_config)},1);x&&document.documentElement.setAttribute("data-Atpl-useragent",ta());pa.sendInstantHCRequest()}};this.halt=function(){b(d.WARNING,"halt, Resetting Atpl");p.i=void 0;Wa=!p.i;V=!1;kb="/i";Sa=
...
this.device_id),w("cly_id_type",1)));a=!1;if(0<H.length)for(var c=0;c<H.length;c++)"[CLY]_temp_id"===H[c].device_id&&(H[c].device_id=this.device_id,a=!0);a&&w("cly_queue",H,!0)}else b(d.WARNING,"disable_offline_mode, Atpl was not in offline mode.")};this.begin_session=function(a,c){b(d.INFO,"begin_session, Starting the session. There was an ongoing session: ["+V+"]");a&&b(d.INFO,"begin_session, Heartbeats are disabled");c&&b(d.INFO,"begin_session, Session starts irrespective of session cookie");
...
a),this.check_consent("star-rating")&&(K?b(d.ERROR,"enableRatingWidgets, Cannot enable rating widgets in offline mode"):(w("cly_fb_widgets",a.popups||a.widgets),Qa(this.url+"/star-rating/stylesheets/Atpl-feedback-web.css"),a=a.popups||a.widgets,0<a.length?(document.body.insertAdjacentHTML("beforeend",'<div id="cfbg"></div>'),this.initializeRatingWidgets(a)):b(d.ERROR,"enableRatingWidgets, You should provide at least one widget id as param. Read documentation for more detail. https://resources.count.ly/plugins/feedback")))):
...
"/star-rating/stylesheets/Atpl-feedback-web.css")}else b(d.DEBUG,"present_feedback_widget, Loading css for survey or nps."),Qa(this.url+"/surveys/stylesheets/Atpl-surveys.css"),t="surveys";n+="?widget_id="+a._id;n+="&app_key="+this.app_key;n+="&device_id="+this.device_id;n+="&sdk_name=javascript_native_web";n+="&platform="+this.platform;n+="&app_version="+this.app_version;n+="&sdk_version=23.12.0";if(h){var I={};I.sg=h;n+="&custom="+JSON.stringify(I)}n+="&origin="+r;n+="&widget_v=web";var C=
document.createElement("iframe");C.src=n;C.name="Atpl-"+t+"-iframe";C.id="Atpl-"+t+"-iframe";var na=!1;C.onload=function(){na&&(document.getElementById("Atpl-"+t+"-wrapper-"+a._id).style.display="none",document.getElementById("csbg").style.display="none");na=!0;b(d.DEBUG,"present_feedback_widget, Loaded iframe.")};for(var Da=document.getElementById("csbg");Da;)Da.remove(),Da=document.getElementById("csbg"),b(d.DEBUG,"present_feedback_widget, Removing past overlay.");var Z=document.getElementsByClassName("Atpl-"+
t+"-wrapper");for(h=0;h<Z.length;h++)Z[h].remove(),b(d.DEBUG,"present_feedback_widget, Removed a wrapper.");Z=document.createElement("div");Z.className="Atpl-"+t+"-wrapper";Z.id="Atpl-"+t+"-wrapper-"+a._id;"survey"===a.type&&(Z.className=Z.className+" "+a.appearance.position);var bb=document.body;h=!1;c&&(document.getElementById(c)?(bb=document.getElementById(c),h=!0):b(d.ERROR,"present_feedback_widget, Provided ID not found."));h||e&&(document.getElementsByClassName(e)[0]?bb=document.getElementsByClassName(e)[0]:
b(d.ERROR,"present_feedback_widget, Provided class not found."));bb.insertAdjacentHTML("beforeend",'<div id="csbg"></div>');bb.appendChild(Z);if("rating"===a.type){var ub=document.createElement("div");ub.className="Atpl-ratings-overlay";ub.id="Atpl-ratings-overlay-"+a._id;Z.appendChild(ub);b(d.DEBUG,"present_feedback_widget, appended the rating overlay to wrapper");A(document.getElementById("Atpl-ratings-overlay-"+a._id),"click",function(){document.getElementById("Atpl-ratings-wrapper-"+
a._id).style.display="none"})}Z.appendChild(C);b(d.DEBUG,"present_feedback_widget, Appended the iframe");A(window,"message",function(B){var T={};try{T=JSON.parse(B.data),b(d.DEBUG,"present_feedback_widget, Parsed response message "+T)}catch(Vb){b(d.ERROR,"present_feedback_widget, Error while parsing message body "+Vb)}T.close?(document.getElementById("Atpl-"+t+"-wrapper-"+a._id).style.display="none",document.getElementById("csbg").style.display="none"):b(d.DEBUG,"present_feedback_widget, Closing signal not sent yet")});
...
(P=!0,m(a))})});break;case "onScrollHalfwayDown":A(window,"scroll",function(){if(!P){var B=Math.max(window.scrollY,document.body.scrollTop,document.documentElement.scrollTop),T=Pa();B>=T/2&&(P=!0,m(a))}});break;default:P||(P=!0,m(a))}}else if("nps"===a.type)document.getElementById("Atpl-"+t+"-wrapper-"+a._id).style.display="block",document.getElementById("csbg").style.display="block";else if("rating"===a.type){var cb=!1;"complete"===document.readyState?cb||(cb=!0,g(a)):A(document,"readystatechange",
...
console.warn("[WARNING] [Atpl] deserialize, Could not parse the file:["+k+"], error: "+q)}return k};p.getViewName=function(){return x?window.location.pathname:"web_worker"};p.getViewUrl=function(){return x?window.location.pathname:"web_worker"};p.getSearchQuery=function(){if(x)return window.location.search};p.DeviceIdType={DEVELOPER_SUPPLIED:0,SDK_GENERATED:1,TEMPORARY_ID:2};la.default=p;Object.defineProperty(la,"__esModule",{value:!0})});
